...
uint32_t enc::aes::subWord(std::uint32_t in) const noexcept {
	uint32_t inw = in;
...
void enc::aes::addRoundKey(void * data, int round)  const noexcept {
...
void enc::aes::subBytes(void * data)  const noexcept {
...
void enc::aes::invSubBytes(void * data)  const noexcept {
	unsigned char *cb = (unsigned char*)data;
	for (int i = 0; i<16; i += 4)//理論的な意味から二重ループにしているが意味は無い
	{
		for (int j = 0; j<4; j++) {
...
void enc::aes::shiftRows(void * data) const noexcept {
	;
...
void enc::aes::invShiftRows(void * data) const noexcept {
	auto ptr{ (uint8_t*)data };
	block_16 cw(data);
	for (int i = 0; i<NB; i += 4) {
		int i4 = i * 4;
...
void enc::aes::mixColumns(uint32_t * data) const noexcept {
	using namespace enc;
	int i, i4, x;
	for (i = 0; i<NB; i++) {
		i4 = i * 4;
...
void enc::aes::invMixColumns(uint32_t * data) const noexcept {
	using namespace enc;
	int i, i4, x;
	for (i = 0; i<NB; i++) {
		i4 = i * 4;
...
enc::aes::aes() : mKey{}, nr{ mKey.keyLength / 4 + 6 } {}

enc::aes::aes(const aesKey & k, unsigned int threadnum) : mKey{ k }, nr{ mKey.keyLength / 4 + 6 } {}

enc::aes::aes(aesKey && k, unsigned int threadnum) : mKey{ k }, nr{ mKey.keyLength / 4 + 6 } {}

void enc::aes::setKey(const aesKey & k) {
	if (k.keyLength % 8 == 0 && k.keyLength / 8 < 1) throw std::runtime_error("invalid keyLength");
	mKey = k;
	nr = mKey.keyLength / 4 + 6;
}

const enc::aesKey & enc::aes::getKey() const noexcept {
	return mKey;
}

std::vector<uint8_t> enc::padData(const void * data, const size_t sizeInByte) {
...
void enc::add(block_16 & a, const block_16 & b) {
	for (int i = 0; i < 16; i++) {
		a.bytes[i] ^= b.bytes[i];
	}
}

void enc::add(void * a, const void * b, const size_t sizeInByte) {
	auto ptr = (uint8_t*)a;
...
std::uint32_t enc::rotWord(std::uint32_t in) {
